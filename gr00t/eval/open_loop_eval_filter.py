...
def plot_trajectory_results(
    ...
    Args:
        ...
        filtered_pred_action_across_time: Low-pass filtered predicted actions over time
...
def low_pass_filter_actions(actions: np.ndarray, alpha: float) -> np.ndarray:
    """Apply a first-order low-pass filter (EMA) along time for each action dim."""
...
def evaluate_single_trajectory(
    ...
    if use_low_pass_filter:
        ...
        logging.info(
            f"Low-pass(alpha={low_pass_alpha}) Action MSE across single traj: {filtered_mse}"
        )
        logging.info(
            f"Low-pass(alpha={low_pass_alpha}) Action MAE across single traj: {filtered_mae}"
        )
...
class ArgsConfig:
    ...
    use_low_pass_filter: bool = False
    """Apply a first-order low-pass filter to predicted actions before plotting."""
